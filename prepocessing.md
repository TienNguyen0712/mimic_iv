...
Một số yêu cầu khi thực hiên bước tiền xử lý
- Giữ lại lần nhập ICU đầu tiên trong mỗi admission của bệnh nhân
- Thời gian nằm ICU tối thiểu 24h đảm bảo đủ dữ liệu cho từng cửa sổ quan sát

## Tạo bảng các bảng trong thư mục **core** 

### Tạo bảng `patient_master`

_Bảng này mô tả thời gian nhập viện, vào ICU của bệnh nhân các thủ tục hành chính theo từng bệnh nhân_

Thực hiện kết hợp hai bảng `admission` và `icustay`: 
- Thực hiện ép kiểu thời của các cột thời gian thành `TIMESTAMP`
- Xử lý các logic như: Thời gian ra viện trước thời gian vào viện hoặc thời gian chuyển viện trước thời gian vào ca của bệnh viện
- Các cột cần lấy đã được liệt kê ở bảng trên và được thêm vào :
  - `admission_time`:  Thời điểm nhập viện
  - `discharge_time`: Thời điểm xuất viện
  - `icu_intime`: Thời gian nhập ICU
  - `icu_outtime`: Xuất ICU

### Tạo bảng `patients_clean`

_Bảng này chứa thông tin lâm sàng của bệnh nhân, giới tính, tuổi_

  - Thực hiện việc lấy giới tính của bệnh nhân
  - `anchor_year`: Năm mà bệnh nhân nhập viện
  - `anchor_age`: Số tuổi của bệnh nhân
  - Và chuyển các cột liên quan tới số thành kiểu `INTERGER`

### Tạo bảng `height_weight`

_Bảng này chưa cân nặng và chiều cao của bệnh nhân_

  - Lấy cân nặng và chiều cao của bệnh nhân
  - Cụ thể kết hợp với bảng `itemid` với mã id là `[226707 (inch), 226512 (kg), 226730 (cm), 226531 (lbs)]`
  - Với bệnh nhân nào có cân nặng là lbs hay chiều cao là inch thì chuyển thành kg và cm cho cùng đơn vị

## Tạo bảng các bảng trong thư mục **clinical**

### Tạo bảng `vital_signs`

_Bảng này chứa thông itn lâm sàng của bệnh nhân_
